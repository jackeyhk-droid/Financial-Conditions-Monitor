...
> **⚠️ 本次(v3 機構級升級)更新了後端** —— `fetch_fred.py` 新增了「準備金 `WRESBAL`、四個 Pillar 分數、資料健康度」三類欄位。**若你的 repo 已在運行,請務必把這三個檔案一起換成新版:`index.html` + `fetch_fred.py` + `data.json`**(三者是一組;只換 index.html,新面板會抓不到資料)。最乾淨做法:先換 `fetch_fred.py` → 到 Actions 手動 Run 一次(`data.json` 會自動重生含全部新欄位)→ 再換 `index.html`。
...
## 第 8 步(選用) — 開啟伺服器端權限(真正的存取控管)

頁面內的通行碼只是**展示用入口(display gate)**:資料其實已在瀏覽器裡,擋不住技術性繞過。若要**真正**的存取控管,有兩條路:

### 路線 A(零程式,最穩) — Vercel 內建保護
Vercel → 你的專案 → **Settings → Deployment Protection** → 開啟 **Password Protection** 或 **Vercel Authentication**。完全不用改檔案,平台層級先擋。(Password Protection 需 Pro;Vercel Authentication 各方案皆有,用 Vercel 帳號登入。)

### 路線 B — 本包附的 `middleware.js`(自訂登入頁、可嵌 iframe)
1. 把 **`middleware.js`** 放到 **repo 根目錄**(和 `index.html` 同一層)。本檔已是**零相依(無任何 import)**,純靜態專案(無 package.json)也能正常部署。
2. Vercel → **Settings → Environment Variables** → 新增 `SITE_PASSWORD` = 你的密碼(例如 `23008133`)→ **Redeploy**。
3. 完成後開啟網址會先看到伺服器端登入頁,輸入密碼後才進入。

**重點:**
- 若 `SITE_PASSWORD` 沒設,middleware 自動失效(不影響部署);設了才啟用。
- Cookie 採 `SameSite=None; Secure`,**嵌在 Squarespace 的 iframe 內也能用**。
- 這層是「真正的邊界」;頁面內通行碼可留作第二層,或移除(刪 `index.html` 的 `#gate` 區塊與其腳本)。兩者可設同一組密碼以免重複輸入。
- 登出:清除該網域 Cookie,或把 `SITE_PASSWORD` 換值並 redeploy。

> **若現在就想先上線、暫時不要伺服器端鎖**:直接**不要放 `middleware.js`**(或從 repo 刪掉)即可 —— Vercel 會正常部署,頁面內通行碼仍照常運作;之後想加再放回。
>
> ⚠️ `middleware.js` **務必用本包這個零相依版本**。早期若放過 `import … from '@vercel/edge'` 的版本,Vercel 在純靜態專案會找不到該套件而**部署失敗**(就是你剛遇到的);換成本版即解決。
