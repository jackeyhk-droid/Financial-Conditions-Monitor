...
頁面內的通行碼只是**展示用入口(display gate)**:資料其實已在瀏覽器裡,擋不住技術性繞過。若要**真正**的存取控管,加上 Vercel Edge Middleware —— 它在邊緣節點先驗證,**通過前不會傳送任何檔案(含 `data.json`)**。

**步驟(約 3 分鐘):**

1. 把壓縮包內的 **`middleware.js`** 放到 **repo 根目錄**(和 `index.html` 同一層)。
2. Vercel → 你的專案 → **Settings → Environment Variables** → 新增:
   - Name = `SITE_PASSWORD`,Value = 你的密碼(例如 `23008133`)。
3. **Redeploy**(Deployments → 最新一筆 → ⋯ → Redeploy)。

完成後,任何人開啟網址會先看到伺服器端登入頁,輸入密碼後才進入。

**重點:**
- 若 `SITE_PASSWORD` 沒設,middleware 自動失效(不影響現有頁面);設了才啟用。
- Cookie 採 `SameSite=None; Secure`,**嵌在 Squarespace 的 iframe 內也能用**。
- 這層是「真正的邊界」;頁面內的通行碼可保留當第二層,或移除(刪掉 `index.html` 內的 `#gate` 區塊與其驗證腳本)。兩者可設同一組密碼以免重複輸入。
- 想登出:清除該網域 Cookie,或把 `SITE_PASSWORD` 換一個值並 redeploy(會讓所有既有 session 失效)。
