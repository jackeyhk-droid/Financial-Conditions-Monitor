...
- **Four Pillar Scores** — risk is split by *type* so a normal-looking composite can't mask one extreme pillar: **Systemic Stress · Duration & Valuation · Cycle & Recession · Liquidity & Funding**. Each is a **robust median/MAD (winsorized) z-score** of its members → sigmoid → 0–100. Each card shows **signed point contributions with direction** (主要壓力 / 主要抵銷 — e.g. *Cycle: credit-momentum −12pts, curve +slope −4pts*) so you see whether a driver is pushing the score up or down, a **coverage indicator** (e.g. *涵蓋 2 項 · 窄* for the narrow Liquidity pillar, so it isn't mistaken for full funding-system capture), and a **dampened 5-day-median WoW** so a one-day quarter-end repo spike can't swing the pillar 20+ points. (This is what surfaces *Duration pressure 63* while the blended composite reads a calm 44.)
...
- **Model Validation tab** — **point-in-time** backtest (expanding-window normalization, 252-day burn-in, *no look-ahead in the signal*), with **strict sample discipline**: baseline, gauge-quintiles, regime stats and the predictor comparison all run on one common eligible mask (gauge-available ∩ forward-available), so the baseline N equals the Σ of the quintile Ns and every predictor is compared on identical dates. Regime rows show **eligible-N / total-N**. Robustness is shown three ways — a **rolling-origin walk-forward** (expanding train, test each subsequent year; reports median test IC, share of folds with negative drawdown-IC, worst fold), a **non-overlapping** (every-63-trading-day) IC, and **year-by-year IC** (the partial current year is dimmed/asterisked). **Honest headline:** the composite is a *coincident* stress/regime classifier, **not** a forward-return predictor — in this sample no robust, monotonic, extrapolable downside-warning ability is observed; the drawdown-IC is negative in only a minority of folds and the non-overlapping sample even flips sign vs the daily one. The dashboard's edge is structural decomposition, not market timing.
